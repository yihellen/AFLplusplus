...
extern s32 forksrv2_pid, child2_pid, fsrv2_ctl_fd, fsrv2_st_fd;
...
extern u8 *out_file, *target_path, *target_path_orig, *target_path2, *doc_path;
...
void init_forkserver2(char **argv) {
  ...
  if (!target_path_orig) target_path_orig = target_path;
  ...
  if (forksrv2_pid < 0) PFATAL("fork() failed");
  ...
  if (!forksrv2_pid) {
    ...
    if (dup2(ctl_pipe[0], FORKSRV_FD + 2) < 0) PFATAL("dup2() failed");
    if (dup2(st_pipe[1], FORKSRV_FD + 3) < 0) PFATAL("dup2() failed");
  ...
  fsrv2_ctl_fd = ctl_pipe[1];
  fsrv2_st_fd = st_pipe[0];
  ...
  rlen = read(fsrv2_st_fd, &status, 4);
  ...
  if (waitpid(forksrv2_pid, &status, 0) <= 0) PFATAL("waitpid() failed");
